...
CapacitiveSensor   cs_5_6 = CapacitiveSensor(5,6); // 1M resistor between pins 2 & 4, pin 4 is sensor pin, add a wire and or foil
...
// ========================================
void setup() {
  // Set touch sensor pins
   pinMode(6, INPUT);
  pinMode(A2, OUTPUT); //LEDS
...
void loop() {
  ...
  {
    if(state == LOW) // turn on the White LED if pin 8
    {
  ...
  if (rtc_time.min % 3 == 0) 
    {
      // Edit: commented this out as this seems to create errors in labels on the screen 
      // (due to the other code that was removed for space)
      //drawVoltage();  
      KeepCalm(3000);  
    }
  drawWeather();
